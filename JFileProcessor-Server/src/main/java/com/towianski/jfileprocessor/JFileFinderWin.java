...
//    }
    ...
    public int getFilesysType() {
        return Constants.FILESYSTEM_POSIX;  //filesysType;
    }
...
//                break;
    ...
    public FilesTblModel getFilesTblModel()
        {
        return (FilesTblModel) filesTbl.getModel();
        }
...
//                System.out.println( "add fpath =" + (String) filesTblModel.getValueAt( rowIndex, FilesTblModel.FILESTBLMODEL_PATH ) + "=" );
    ...
    private void initComponents()
    {
        ...
        filesTbl = new javax.swing.JTable()
        {
            ...
            gridBagConstraints.insets = new java.awt.Insets(2, 5, 4, 0);
...
//            for( int row : filesTbl.getSelectedRows() )
            for( int row=0; row < 1; row++ ) 
                ...
                try {
//                    ScpTo scpTo = new ScpTo();
//                    scpTo.copyTo( file.toString(), "kids", "hostess", "localhost", DesktopUtils.getUserTmp() + System.getProperty( "file.separator") + file.getName().toString() );
                    scpTo.copyTo( "/net2/github/JFileProcessor2/build/libs/JFileProcessor.jar-1.5.11.jar", "stan", "yearly55", "192.168.56.103", "/opt/jFileProcessor/JFileProcessor.jar-1.5.11.jar" );
            System.out.println( "try scpTo   file =" + file + "=   to remote =kids@localhost:" + DesktopUtils.getUserTmp() + System.getProperty( "file.separator") + file.getName().toString() + "=" );
                    } 
                catch (Exception ex) 
                    {
                    Logger.getLogger(JFileFinderWin.class.getName()).log(Level.SEVERE, null, ex);
...
//            scpTo.exec( "stan", "yearly55", "192.168.56.103" );
    ...
    public static void main(String args[]) {
        ...
        if ( args.length > 1 && args[1].equals( "-rest" ) )
            {
            System.out.println( "args[1] =" + args[1] + "=" );
            System.out.println( "DO NOT START GUI" );
            startGui = false;
            }
        if ( startGui )
            {
