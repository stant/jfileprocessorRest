...
//     def loader = new RootLoader( urls, this.class.classLoader)
    ...
    watchDirToCallerEventQueue = new WatchDirToCallerEventQueue( "watchFolders-2", codeProcessorPanel.jFileFinderWin.getWatchFileEventsSw() , pathList, 3000, fileEventQueue );
    ...
    Thread qThread = ProcessInThread.newThread( "watchDirToCallerEventQueue-watchFolders-2", 0, true, watchDirToCallerEventQueue );
    qThread.start();
    ...
    try {
        while (true) 
            ...
            try {
                Files.move( fte.getFullFilePath(), 
                    Paths.get("/net2/watch-3").resolve( Paths.get( fte.getFilename() ) ), 
        ...
        if ( qThread != null )
            {
            System.out.println( "watchFolders2 - before qThread.join()" );
            if ( qThread.isAlive() )
                {
                try
                    {
            ...
            System.out.println( "watchFolders2 - after qThread.join()" );
            }
        }
