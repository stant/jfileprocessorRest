...
//   }
    ...
    public Copier( Boolean isDoingCutFlag, ArrayList<CopyOption> copyOptions )   //, CopyFrameSwingWorker swingWorker )
    {
        ...
        this.copyOptions = copyOptions.toArray( new CopyOption[ copyOptions.size() ] );
...
//                Files.copy( file, toPathFile );
                ...
                {
                System.out.println("copy with sent copyoptions. file =" + file + "=   to =" + toPath.resolve(startingPath.relativize( file ) ) + "=" );
